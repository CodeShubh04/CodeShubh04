# 👋 Hi, I'm Shubhanshi Verma

🎓 **B.Tech CSE graduate | Data Analyst | AI & ML Enthusiast**  
💡 Passionate about turning data into insights that drive smarter decisions.  

---

### 🧠 About Me  
- 🔍 I enjoy exploring data to find patterns, insights, and solutions.  
- 💻 Skilled in Python, SQL, Excel, Tableau, and Power BI.  
- 🌱 Currently exploring AI tools and their use in business and startups.  
- 💬 I love discussing data storytelling, visualization, and problem-solving.  

---

### 🚀 Projects I'm Proud Of  

📊 **[Customer Churn Prediction](https://github.com/CodeShubh04/Customer-Churn-Prediction)**  
Built a logistic regression model with PCA & LDA for dimensionality reduction; achieved 85% accuracy with visualization insights.  

🍴 **[Zomato Customer Relationship Analysis](https://github.com/CodeShubh04/Zomato-CRM-Analysis)**  
Performed end-to-end data cleaning, analysis, and insights extraction using Pandas and Matplotlib.  

💸 **[Expense Tracker](https://github.com/CodeShubh04/Expense-Tracker)**  
Developed an interactive Python + Excel tracker to monitor and visualize monthly expenses.  

🤖 **[Gender and Age Detection](https://github.com/CodeShubh04/Gender-and-Age-Detection)**  
CNN-based model using OpenCV to predict gender and age from facial images.  

---

### 🛠️ Tech Stack  

| Category | Tools |
|-----------|-------|
| **Languages** | Python, SQL, C++, Java |
| **Libraries** | NumPy, Pandas, Matplotlib, Seaborn, Scikit-learn |
| **Data Tools** | Excel, Tableau, Power BI |
| **Other Skills** | Data Analysis, Visualization, EDA, Machine Learning, Statistical Modeling |
| **Soft Skills** | Analytical Thinking, Problem Solving, Communication, Collaboration |

---

### 🧩 Experience  

💼 **Data Science Intern — CodeClause**  
- Built a Gender and Age Detection Model using CNN and OpenCV.  
- Learned how to manage datasets and fine-tune deep learning models.  

✍️ **Technical Writer Intern — Freaky Diodes**  
- Authored technical blogs and AI content to simplify data and ML concepts.  

👥 **Member — Google Developer Student Clubs (GDSC)**  
- Participated in workshops and team projects promoting AI & data literacy.  

---

### ⚙️ GitHub Analytics  
 
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=CodeShubh04&layout=compact&theme=radical)

---

### 📫 Connect with Me  

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/shubh-verma/)  
[![GitHub](https://img.shields.io/badge/GitHub-black?style=flat&logo=github)](https://github.com/CodeShubh04)  
[![Email](https://img.shields.io/badge/Email-grey?style=flat&logo=gmail)](mailto:codeshubh04@gmail.com)

---

⭐ _“Data tells stories — I turn them into insights.”_
